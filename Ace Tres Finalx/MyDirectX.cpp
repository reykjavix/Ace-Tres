...
void SetCamera(float posx, float posy, float posz, float lookx, float looky, float lookz)
{
    ...
    float aspectRatio = 1.2;//SCREENW / SCREENH;
    float nearRange = 1.0;
...
/* Translate screen coordinates to world coordinates. Assumes camera is at origin. */
D3DXVECTOR3 screenToWorld(float x, float y, float z)
{
	...
	/* Get viewport to pull out exact screen size, get the projection matrix, get the view matrix */
	d3ddev->GetViewport( &m_mainViewport );
	d3ddev->GetTransform(D3DTS_PROJECTION, &matProj);
	d3ddev->GetTransform(D3DTS_VIEW, &matView);

	D3DXVECTOR3 v;
	v.x =  ( ( ( 2.0f * x ) / m_mainViewport.Width  ) - 1 ) / matProj._11;
	v.y = -( ( ( 2.0f * y ) / m_mainViewport.Height ) - 1 ) / matProj._22;
	v.z =  1.0f;
	...
	/* scale ray to desired z value */
	float scale = z/rayDir.z;
	return scale * rayDir;
}